...
then
    while [ $((PORT < ENDPORT)) != "0" ]; do
        PORT=$((PORT+1))
        echo "Starting $PORT"
        redis-server --port $PORT --protected-mode $PROTECTED_MODE \
        --cluster-enabled yes \
        --cluster-config-file redisnodes-${PORT}.conf \
        --cluster-node-timeout $TIMEOUT \
        --appendonly yes \
        --appendfilename appendonly-${PORT}.aof \
        --appenddirname appendonlydir-${PORT} \
...
then
    ...
    redis-cli --cluster create $HOSTS --cluster-replicas $REPLICAS $OPT_ARG
    exit 0
fi
